...
return row;
        ...
        public static bool ReadBool(object value)
        {
            ...
            int s = temp == "0" || temp == "false" ? 0 : temp == "1" || temp == "true" ? 1 : 2;
            if (s == 2) throw new ArgumentException("Unable to read value as boolean", "value");
            return s == 1;
        }
        [Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete("This method is for internal usage only", false)]
        public static bool? ReadNullableBool(object value)
        {
            ...
            string temp = value.ToString().ToLower();
            int s = temp == "0" || temp == "false" ? 0 : temp == "1" || temp == "true" ? 1 : 2;
            if (s == 2) throw new ArgumentException("Unable to read value as nullable boolean", "value");
            return s == 1;
        }
        public static Guid ReadGuid(object value)
        {
            ...
            bool temp = Guid.TryParse(value.ToString(), out result);
            if (!temp) throw new ArgumentException("Unable to read value as Guid", "value");
            return result;
        }
        [Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete("This method is for internal usage only", false)]
        public static Guid? ReadNullableGuid(object value)
        {
            ...
            Guid result = Guid.Empty;
            bool temp = Guid.TryParse(value.ToString(), out result);
            if (!temp) throw new ArgumentException("Unable to read value as nullable Guid", "value");
            return result;
        }
        /// <summary>
        /// Internal use only
...
)
        {
            ...
            foreach (var item in members)
            {
                if (item != null)
                {
                    ...
                    {
                        il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod(
                            memberType == typeof(Guid) ? "ReadGuid" : "ReadNullableGuid", BindingFlags.Static | BindingFlags.Public), null); // stack is now [target][target][typed-value]
                    }
                    else
                    {
